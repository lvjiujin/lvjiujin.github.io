...
Let $X$ denote the word-word co-occurrence matrix, where $X_{ij}$ indicates the number of times word $j$ occur in the context of word $i$. Let $X_i = \sum{_k}X_{ik}$ be the number of times any word k appears in the context of word i. Finally, let $P_{ij} = P({w_j}|{w_i}) = \frac{X_{ij}}{X_i}  $ be the probability of j appearing in the context of word i.
